...
void OnBuy()
{
    if (currentShopTab == ShopTab.SKINS)
    {
        if (gameData.HasSkinInInventory(CurrentSkin.id))
        {
...
{
    BackButton.gameObject.SetActive(currentObject > 0);
    NextButton.gameObject.SetActive(currentObject < objects.Length - 1);             
}
